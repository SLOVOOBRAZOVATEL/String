...
char *s21_strerror(int errnum) {
  char *ptr = s21_NULL;
  const char **maserror = get_error_messages();  // Получаем массив ошибок
  if (errnum >= MAX_ERRLIST || errnum <= MIN_ERRLIST) {
    static char part1[64] =
        ERROR;  // Увеличиваем размер для корректного хранения
    static char part2[33] = {0};
    s21_memset(part1 + s21_strlen(ERROR), 0, sizeof(part1) - s21_strlen(ERROR));
